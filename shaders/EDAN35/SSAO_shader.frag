...
void main()
{
	...
	for(int i = 0; i < kernelSize; ++i)
	{
		...
		vec4 world_space_depth_pos_temp = view_projection_inverse * vec4(stored_depth_pos, 1.0); //Almost world space
		vec3 world_space_depth_pos = world_space_depth_pos_temp.xyz / world_space_depth_pos_temp.w; //World space
		vec4 view_space_depth_pos = WorldToViewMatrix * vec4(world_space_depth_pos, 1.0); //View space

		if (ssao_range_check) {
			float rangeCheck = smoothstep(0.0, 1.0, radius / abs(view_space_depth_pos.z - sample_pos.z));
			occlusion += (view_space_depth_pos.z >= sample_pos.z + bias ? 1.0 : 0.0) * rangeCheck;
		} else {
			occlusion += (view_space_depth_pos.z >= sample_pos.z + bias ? 1.0 : 0.0);
		}
	}
