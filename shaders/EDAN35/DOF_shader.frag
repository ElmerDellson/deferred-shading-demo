...
void main() {
	...
	//Simple crosshair for debugging purposes
	vec2 crosshair = (1 / inv_res) / 2.0;

	//Calculate the Circle of Confusion using the parameters of our "virtual camera"
	float A = aperture;
	...
	float CoC = abs(A * (F * (P - D)) / (D * (P - F))); //Calculate Circle of Confusion

	//Blurring based on the CoC
	...
	if (CoC >= 2) {
		...
		for (int x = -(samples/2); x < (samples/2); ++x)
		{
			for (int y = -(samples/2); y < (samples/2); ++y)
			{
				...
				if (DOF_range < length(P - sample_view_space_pos.z)) { // || (400.0 > length(P - sample_view_space_pos.z))) {
